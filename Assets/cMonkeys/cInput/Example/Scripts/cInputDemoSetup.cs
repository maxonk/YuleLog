...
public class cInputDemoSetup : MonoBehaviour {
	...
	void Start() {
        ...
        cInput.SetKey("Drag Objects", "Mouse0");
        cInput.SetKey("Scale Log Up", "Mouse Wheel Up");
        cInput.SetKey("Scale Log Down", "Mouse Wheel Down");
        cInput.SetKey("Get Log", "Mouse1");

        // we define an axis like this:
        cInput.SetAxis("Scale Log", "Scale Log Up", "Scale Log Down"); // sets the 'Pause' input to "P" - notice we didn't set up a secondary input-this will be defaulted to 'None'
       }
}
